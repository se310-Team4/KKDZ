...
describe("the homepage", () => {
  ...
  it("links redirect to its web page", () => {
    for (let i = 0; i < miniGames.length; i++) {
      cy.visit("http://localhost:1234");
      cy.get(`span[id="${miniGames[i]}-btn"]`).should("contain.text", miniGameTitles[i]);
      cy.get(`span[id="${miniGames[i]}-btn"]`).click();
  ...
  it("should display an image in all buttons", () => {
    for (const game of miniGames) {
      cy.get(`span[id="${game}-btn"]`).find("img").should("be.visible");
    }
  });

  it("should display details when button is hovered", () => {
    for (const game of miniGames) {
      cy.get(`span[id="${game}-btn"]`).realHover("mouse");
